...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("EquiTrader-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EquiTraderTEST2\r");
            Send(hSocket, "WHO #EquiTraderTEST2\r");
        } else {
            // randomly join #EquiTrader00-#EquiTrader05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #EquiTrader%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EquiTrader%02d\r", channel_number).c_str());
        }
