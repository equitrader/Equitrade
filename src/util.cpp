...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EquiTrader
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EquiTrader
    // Mac: ~/Library/Application Support/EquiTrader
    // Unix: ~/.EquiTrader
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EquiTraderData";
#else
    fs::path pathRet;
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("EquiTrader"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
